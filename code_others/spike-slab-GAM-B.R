...
sp_sl_model_B <- spikeAndSlab(y, X, 
...
for (s in sample) {
  ...
  # plot(sp_sl_model_B$samples$beta[, s], main = bquote(beta[.(s)])
  
  # Assuming 'samples' is your MCMC sample matrix or data frame
  mcmc_chain_list <- sp_sl_model_B$samples$beta[, s]
