...
pheno = 'a_lab'                  # Specify the phenotype of interest
...
spike_slab_model_a_lab <- lm.spike(a_lab ~ ., data = df1, niter = niter,
...
coef_df <- summary(spike_slab_model_a_lab)$coef
...
spike_slab_model_predict <- predict(spike_slab_model_a_lab, df_test, burn = burn)

print("Saving the model outputs")
save(spike_slab_model_a_lab, file = file)
