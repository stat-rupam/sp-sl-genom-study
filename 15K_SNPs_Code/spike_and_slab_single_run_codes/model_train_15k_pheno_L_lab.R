...
pheno = 'L_lab'                  # Specify the phenotype of interest
...
spike_slab_model_L_lab <- lm.spike(L_lab ~ ., data = df1, niter = niter,
...
coef_df <- summary(spike_slab_model_L_lab)$coef
...
spike_slab_model_predict <- predict(spike_slab_model_L_lab, df_test, burn = burn)

print("Saving the model outputs")
save(spike_slab_model_L_lab, file = file)
