...
pheno = 'G'                  # Specify the phenotype of interest
...
spike_slab_model_G <- lm.spike(G ~ ., data = df1, niter = niter,
                               prior.information.weight = prior_information_weight,
                               diagonal.shrinkage = diagonal_shrinkage)
print("Model Training Done")
coef_df <- summary(spike_slab_model_G)$coef
coef_df <- data.frame(coef_df)
print("Coefficeint extraction form the model output done")
file = paste0("spike_slab_output_15K_",pheno,".RData")
print("Pridction genration started")

df_test <- testing_set[,-c(1:10)]  # Extract the predictor variables               # Specify the phenotype of interest
df_test[[pheno]] <- testing_set[[pheno]]
spike_slab_model_predict <- predict(spike_slab_model_G, df_test, burn = burn)

print("Saving the model outputs")
save(spike_slab_model_G, file=file)
