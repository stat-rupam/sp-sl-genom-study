...
pheno = 'B'                  # Specify the phenotype of interest
...
spike_slab_model_B <- lm.spike(B ~ ., data = df1, niter = niter,
...
coef_df <- summary(spike_slab_model_B)$coef
...
spike_slab_model_predict <- predict(spike_slab_model_B, df_test, burn = burn)

print("Saving the model outputs")
save(spike_slab_model_B, file = file)
