...
pheno = 'C'                  # Specify the phenotype of interest
...
spike_slab_model_C <- lm.spike(C ~ ., data = df1, niter = niter,
...
coef_df <- summary(spike_slab_model_C)$coef
...
spike_slab_model_predict <- predict(spike_slab_model_C, df_test, burn = burn)

print("Saving the model outputs")
save(spike_slab_model_C, file = file)
