...
bayesR_model <- ibrm(y~.,data=df,M=X,M.id=1:length(y),method="BayesR"
                    ,niter = 25000
                    ,nburn = 5000)
beta<-bayesR_model$beta
setwd("~/personal/spike-slab-analysis/reusable_functions/simulation_studies")
# Save the fitted model to a file for later use
file_name = paste0("bayesR_model_K_",K,"_theta_",(100*theta),".RData")
save(bayesR_model, file = file_name)
stopCluster(cl)